 ...
 * the {@link theme_ella_less_variables()} here because we need to create selectors or
 * alter existing ones.
 *
 * @param theme_config $theme The theme config object.
 * @return string Raw LESS code.
 */
function theme_ella_extra_less($theme) {
    $content = '';
 ...
 */
function theme_ella_less_variables($theme) {
    $variables = array();
    ...
    if (!empty($theme->settings->phovercolor)) {
        $variables['phovercolor'] = $theme->settings->phovercolor;
    }
 ...
 */
function theme_ella_process_css($css, $theme) {
    global $OUTPUT;

    // Set the background image for the logo.
    $logo = $OUTPUT->get_logo_url(null, 100);
    $css = theme_ella_set_logo($css, $logo);
    ...
    $css = theme_ella_set_customcss($css, $customcss);
 ...
 */
function theme_ella_set_logo($css, $logo) {
    $tag = '[[setting:logo]]';
    $replacement = $logo;
 ...
 * @return bool
 */
function theme_ella_pluginfile($course, $cm, $context, $filearea, $args, $forcedownload, array $options = array()) {
    if ($context->contextlevel == CONTEXT_SYSTEM &&
            ($filearea === 'logo' || $filearea === 'smalllogo' || $filearea === 'backgroundimage')) {
        $theme = theme_config::load('ella');
 ...
 */
function theme_ella_set_customcss($css, $customcss) {
